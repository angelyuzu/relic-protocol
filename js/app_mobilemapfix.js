...
let state=loadState();
(function enableTrialModeNow(){
  const KEY="v94_enable_trial_mode_now_20260726";
  try{
    if(localStorage.getItem(KEY)==="done")return;
    state.campaign=state.campaign||{};
    state.campaign.mode="test";
    localStorage.setItem(STORAGE_KEY,JSON.stringify(state));
    localStorage.setItem(KEY,"done");
  }catch(e){
    console.warn("Could not enable Trial mode automatically.",e);
  }
})();
(function applyJewelryGolfSwapVerifiedV2(){
  const KEY="v94_jewelry_golf_swap_verified_v2_20260725";
  try{
    if(localStorage.getItem(KEY)==="done")return;
    ...
    console.warn("Verified Jewelry/Golf timing migration failed",e);
  }
})();
(function applyJewelryGolfSwapMigration(){
  const KEY="v94_jewelry_golf_swap_20260725";
  try{
    if(localStorage.getItem(KEY)==="done")return;
    state.trialOverrides=state.trialOverrides||{};
    state.codeOverrides=state.codeOverrides||{};
    state.rcLabelOverrides=state.rcLabelOverrides||{};
    [30,35].forEach(id=>{
      delete state.trialOverrides[id]; delete state.trialOverrides[String(id)];
      delete state.codeOverrides[id]; delete state.codeOverrides[String(id)];
      delete state.rcLabelOverrides[id]; delete state.rcLabelOverrides[String(id)];
    });
    localStorage.setItem(STORAGE_KEY,JSON.stringify(state));
    localStorage.setItem(KEY,"done");
  }catch(e){console.warn("Jewelry/Golf migration failed",e)}
})();
  ...
  <div class="card glass condo-map-card"><div class="eyebrow">${ICONS.map} Tactical Condo Map</div><h2>Guild Headquarters Sector Network</h2><p class="muted">Mapped from the condo layout. Real room names remain encrypted; only operational codenames are displayed.</p>
    <div class="condo-map-wrap">
      <svg class="condo-map" viewBox="0 0 1000 1020" role="img" aria-label="Condo tactical sector map">
        ...
        ${lines.map(([a,b])=>`<line x1="${nodes[a][0]}" y1="${nodes[a][1]}" x2="${nodes[b][0]}" y2="${nodes[b][1]}" class="map-line" stroke="rgba(105,220,255,.48)" stroke-width="2.5" stroke-linecap="round" stroke-dasharray="7 8"/>`).join("")}
...
/* Absolute live guarantee for the requested schedule swap.
   Does not touch completed[], XP, Glimmer, stats, Guardian profile, or any other trial. */
(function forceJewelryGolfTiming(){
  const jewelry=TRIALS[29];
  Object.assign(jewelry,{
    reward:"Jewelry Box",
    rarity:"Legendary",
    ...
    sourceType:"sector",
    sector:"Alpha",
    hideSpot:"Closet shelf beside plushies",
    rcLabel:"RC10",
    code:"K535",
    glimmer:0
  });

  const golf=TRIALS[34];
  Object.assign(golf,{
    reward:"Mini Golf Voucher",
    rarity:"Rare",
    category:"Vouchers",
    iconKey:"vouchers",
    sourceType:"depot",
...
/* ===== V9.4 MAP + VAULT ABSOLUTE FINAL OVERRIDES ===== */
function v94SectorRecoveredItems(sector){
  return TRIALS.filter(t=>t.sector===sector && state.completed.includes(t.id));
}
function v94SectorDetail(sector){
  const status=sectorState(sector);
  const revealed=status==="visited"||status==="current";
  if(!revealed){
    return `<div class="sector-detail encrypted-detail"><div class="eyebrow">SECTOR ENCRYPTED</div><h3>Unknown Location</h3><p class="muted">Complete a mission routed here to decrypt this sector.</p></div>`;
  }
  const recovered=v94SectorRecoveredItems(sector);
  return `<div class="sector-detail">
    <div class="sector-detail-head"><div><div class="eyebrow">SECTOR ${sector}</div><h3>${SECTOR_LORE_NAMES[sector]||sector}</h3></div><span class="sector-count">${recovered.length} recovered</span></div>
    <div class="sector-found-list">${recovered.length?recovered.map(t=>`<div class="sector-found-item"><img src="assets/icons/${t.iconKey}.svg" alt=""><div><b>${escapeHtml(t.reward)}</b><small>${getRcLabel(t)} · ${t.rarity}</small></div></div>`).join(""):`<p class="muted">No relics recovered here yet.</p>`}</div>
  </div>`;
}
function v94MapMarkup(){
  const nodes={
    Alpha:[500,70],Bravo:[275,205],Delta:[725,205],Kilo:[820,390],Charlie:[500,390],
    Echo:[210,590],Foxtrot:[405,690],Golf:[710,590],Juliet:[625,800],Hotel:[825,800]
  };
  const lines=[
    ["Alpha","Charlie"],["Bravo","Charlie"],["Delta","Charlie"],["Kilo","Charlie"],
    ["Charlie","Echo"],["Charlie","Foxtrot"],["Charlie","Golf"],["Golf","Juliet"],["Golf","Hotel"]
  ];
  return `<div class="interactive-map-shell"><div class="condo-map-wrap">
    <svg class="condo-map interactive-condo-map" viewBox="0 0 1000 900" role="img" aria-label="Interactive tactical map">
      ${lines.map(([a,b])=>`<line x1="${nodes[a][0]}" y1="${nodes[a][1]}" x2="${nodes[b][0]}" y2="${nodes[b][1]}" class="map-line" stroke="rgba(105,220,255,.48)" stroke-width="2.5" stroke-linecap="round" stroke-dasharray="7 8"/>`).join("")}
      ${Object.entries(nodes).map(([s,[x,y]])=>{
        const status=sectorState(s),revealed=status==="visited"||status==="current",count=v94SectorRecoveredItems(s).length;
        return `<g class="map-node map-node-clickable ${status}" data-map-sector="${s}" transform="translate(${x},${y})">
          <circle r="58" class="node-pulse"/><circle r="50"/><circle r="39" class="inner"/>
          <text class="node-icon" y="-9">${revealed?SECTORS[s].icon:"⌁"}</text>
          <text class="node-code" y="17">${revealed?s:"???"}</text>
          <text class="node-name" y="74">${revealed?(SECTOR_LORE_NAMES[s]||s):"ENCRYPTED"}</text>
          ${revealed&&count?`<g class="map-count-badge" transform="translate(38,-38)"><circle r="14"/><text y="4">${count}</text></g>`:""}
        </g>`;
      }).join("")}
    </svg></div><div class="map-detail-slot" data-map-detail><div class="sector-detail map-instruction"><div class="eyebrow">TACTICAL ARCHIVE</div><h3>Tap a Sector</h3><p class="muted">Decrypted sectors show the relics recovered there.</p></div></div></div>`;
}
function v94BindMap(root){
  const detail=root.querySelector("[data-map-detail]");
  root.querySelectorAll("[data-map-sector]").forEach(node=>node.onclick=()=>{
    root.querySelectorAll("[data-map-sector]").forEach(n=>n.classList.toggle("selected",n===node));
    if(detail) detail.innerHTML=v94SectorDetail(node.dataset.mapSector);
  });
}
renderMap=function(screen){
  screen.innerHTML=`<div class="card glass condo-map-card"><div class="eyebrow">${ICONS.map} Tactical Map // MAP 2.0</div><h2>Guild Headquarters Sector Network</h2><p class="muted">Tap any decrypted sector to review what you've already recovered there.</p>${v94MapMarkup()}<div class="map-legend"><span><i class="dot current"></i> Active</span><span><i class="dot visited"></i> Discovered</span><span><i class="dot locked"></i> Encrypted</span></div></div>`;
  v94BindMap(screen);
};

function v94VaultItem(t){
  return `<div class="vault-card rarity-card ${rarityClass(t.rarity)}"><div class="vault-card-top"><img class="vault-vector" src="assets/icons/${t.iconKey}.svg" alt=""><span class="badge ${rarityClass(t.rarity)}">${t.rarity}</span></div><h3>${escapeHtml(t.reward)}</h3><div class="vault-meta"><span>${ICONS[t.category]||"◫"} ${t.category}</span><span>${getRcLabel(t)}</span></div><small class="muted">${sourceLabel(t)}</small></div>`;
}
function v94VaultGroups(items,mode){
  const groups=mode==="type"?["Credits","Equipment","Provisions","Vouchers","Miscellaneous"]:["Exotic","Legendary","Rare","Uncommon","Common"];
  return groups.map(g=>{
    const matches=items.filter(t=>mode==="type"?t.category===g:t.rarity===g);
    return `<details class="vault-rarity-group ${mode==="type"?"type-group":rarityClass(g)}" ${matches.length?"open":""}><summary><span class="rarity-summary-left"><b>${g}</b></span><span class="rarity-count">${matches.length}</span></summary><div class="detail-body grid g3 vault-rarity-grid">${matches.length?matches.map(v94VaultItem).join(""):`<div class="empty-state">No ${g.toLowerCase()} relics yet.</div>`}</div></details>`;
  }).join("");
}
renderVault=function(screen){
  const unlocked=TRIALS.filter(t=>state.completed.includes(t.id));
  const rarities=["Exotic","Legendary","Rare","Uncommon","Common"];
  screen.innerHTML=`<div class="vault-header-row"><div><div class="eyebrow">${ICONS.vault} Recovered Inventory // VAULT 2.0</div><h2>Vault</h2></div><div class="vault-total"><small>RECOVERED</small><b>${unlocked.length}/40</b></div></div>
  <div class="vault-rarity-summary">${rarities.map(r=>`<div class="vault-summary-chip ${rarityClass(r)}"><small>${r}</small><b>${unlocked.filter(t=>t.rarity===r).length}</b></div>`).join("")}</div>
  <div class="vault-sort-row"><span>SORT INVENTORY</span><div class="vault-sort-toggle"><button class="vault-sort-btn active" data-vault-sort="rarity">Rarity</button><button class="vault-sort-btn" data-vault-sort="type">Type</button></div></div>
  <div id="vaultGroups" class="vault-rarity-stack">${v94VaultGroups(unlocked,"rarity")}</div>`;
  const host=screen.querySelector("#vaultGroups");
  screen.querySelectorAll("[data-vault-sort]").forEach(btn=>btn.onclick=()=>{
    screen.querySelectorAll("[data-vault-sort]").forEach(x=>x.classList.toggle("active",x===btn));
    host.innerHTML=v94VaultGroups(unlocked,btn.dataset.vaultSort);
  });
};
console.info("V9.4 MAP 2.0 + VAULT 2.0 final overrides loaded");


/* Mobile map drag-to-pan */
function enableTacticalMapPan(root){
  const wrap=root.querySelector(".condo-map-wrap");
  ...
  wrap.addEventListener("pointermove",e=>{
    ...
    if(moved){
      wrap.scrollLeft=left-dx;
      wrap.scrollTop=top-dy;
    }
  });
...
const _v94BindMapOriginal=v94BindMap;
v94BindMap=function(root){
  _v94BindMapOriginal(root);
  enableTacticalMapPan(root);
};
